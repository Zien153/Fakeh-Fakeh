...
export default function App(): React.ReactElement {
  // Resume generation logic (replaces 20+ useState calls)
  const [resumeState, resumeActions] = useResumeGeneration();

  // UI navigation logic (replaces 8+ useState calls)
  ...
  const handleExportPdf = async (): Promise<void> => {
    ...
    try {
      ...
      const candidateName = (resumeState.resumeData.personalInfo.fullName || 'Candidate').replace(
        /\s+/g,
        '_'
      );
  ...
  // Context value for avoiding prop drilling
    ...
    <AppProvider value={appContextValue}>
      <div className="min-h-screen bg-slate-100 flex flex-col font-sans" dir="rtl">
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 flex-1 w-full space-y-6">
          ...
          {resumeState.noticeMessage && (
            <div className="no-print p-4 rounded-xl bg-indigo-50 border border-indigo-200 text-indigo-900 text-xs sm:text-sm flex items-center justify-between animate-in fade-in">
              ...
              <button
                onClick={resumeActions.clearNotice}
          ...
          {resumeState.errorMessage && (
            <div className="no-print p-4 rounded-xl bg-rose-50 border border-rose-200 text-rose-900 text-xs sm:text-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  type="button"
                  onClick={() => resumeActions.generate()}
                ...
                <button
                  onClick={resumeActions.clearError}
            ...
            <section id="output-section" className="space-y-4 pt-2">
              ...
              <div className="no-print bg-white p-2 sm:p-2.5 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between flex-wrap gap-2">
                <div className="flex items-center gap-1.5 sm:gap-2">
                  <button
                    type="button"
                    onClick={() => uiActions.setActiveOutputTab('resume')}
                  ...
                  <button
                    type="button"
                    onClick={() => uiActions.setActiveOutputTab('cover-letter')}
                    className={`px-3.5 sm:px-4 py-2 text-xs sm:text-sm font-bold rounded-xl transition-all flex items-center gap-2 cursor-pointer ${
                      uiState.activeOutputTab === 'cover-letter'
                        ? 'bg-indigo-600 text-white shadow-sm'
                        : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100'
                    }`}
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => uiActions.setActiveOutputTab('ats-review')}
                    className={`px-3.5 sm:px-4 py-2 text-xs sm:text-sm font-bold rounded-xl transition-all flex items-center gap-2 cursor-pointer ${
                      uiState.activeOutputTab === 'ats-review'
                        ? 'bg-indigo-600 text-white shadow-sm'
                        : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100'
                    }`}
                  >
                    <span>3. مراجعة وتدقيق كلمات ATS</span>
                    {resumeState.atsReviewData && (
                      <span className="bg-emerald-500 text-white text-[10px] font-black px-1.5 py-0.5 rounded-full">
                        {resumeState.atsReviewData.matchScore}%
                      </span>
                    )}
                  </button>
                </div>
              ...
              {uiState.activeOutputTab === 'resume' && (
                <ResumeView
                  ...
                  atsReview={resumeState.atsReviewData}
                  onUpdateResume={resumeActions.updateResume}
                  targetJobTitle={resumeState.targetJobTitle}
                  targetJobDescription={resumeState.targetJobDescription}
                  onPrint={handlePrint}
                />
              ...
              {uiState.activeOutputTab === 'cover-letter' && resumeState.coverLetterData && (
                <CoverLetterView
                  coverLetter={resumeState.coverLetterData}
                  personalInfo={resumeState.resumeData.personalInfo}
                  onPrint={handlePrint}
                />
              )}
              ...
              {uiState.activeOutputTab === 'ats-review' && resumeState.atsReviewData && (
                <>
                  <AtsReviewCard
                    atsReview={resumeState.atsReviewData}
                    onOpenComparison={() => uiActions.openComparisonSidebar()}
                    onOpenHeatmap={() => uiActions.openHeatmapModal()}
                  />
                  <AtsComparisonSidebar
                    isOpen={uiState.isComparisonSidebarOpen}
                    onClose={() => uiActions.closeComparisonSidebar()}
                    resume={resumeState.resumeData}
                    atsReview={resumeState.atsReviewData}
                    onUpdateResume={resumeActions.updateResume}
                    onNavigateToSection={() => {
                      uiActions.setActiveOutputTab('resume');
                      uiActions.closeComparisonSidebar();
                    }}
                  />
                  <KeywordHeatmapModal
                    isOpen={uiState.isHeatmapModalOpen}
                    onClose={() => uiActions.closeHeatmapModal()}
                    resume={resumeState.resumeData}
                    atsReview={resumeState.atsReviewData}
                    targetJobDescription={resumeState.targetJobDescription}
        ...
        <JobDescriptionPromptModal
          ...
          onSubmit={(desc) => {
            resumeActions.setTargetJobDescription(desc);
            uiActions.closePromptModal();
            resumeActions.generate(desc);
          }}
        />
